...
GL3Sampler::GL3Sampler(GL3Device& device, const SamplerInfo& info)
    ...
    m_device.runWithContext([&](gl3::GLContext* ctx) {
        ...
        GL_CHECK(
            glSamplerParameteri(m_handle, GL_TEXTURE_WRAP_S, g_addressType[(int)info.addressU]));
        GL_CHECK(
            glSamplerParameteri(m_handle, GL_TEXTURE_WRAP_T, g_addressType[(int)info.addressV]));
        GL_CHECK(
            glSamplerParameteri(m_handle, GL_TEXTURE_WRAP_R, g_addressType[(int)info.addressW]));

        GLint minFilter = g_addressType[(int)info.minFilter] | g_addressType[(int)info.mipFilter];
        ...
        GL_CHECK(glSamplerParameterfv(m_handle, GL_TEXTURE_BORDER_COLOR, borderColor));
#if OPENGL_RESOURCE_ANSC
    });
#endif
}
...
GL3Sampler::~GL3Sampler() {
    ...
    if (handle) {
        ...
        m_device.runWithContext([=](gl3::GLContext* ctx) {
#endif
            GL_CHECK(glDeleteSamplers(1, &handle));
#if OPENGL_RESOURCE_ANSC
        });
#endif
    }
