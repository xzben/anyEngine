...
public:
    ...
    void init(Texture* texture, const TextureUpdateInfo& info,
    ...
    virtual void execute(gl3::GLContext* context) override {
        ...
        GL3Texture::getPixelFormatInfo(info.format, &internalFormat,
                                       &pixelFomrat, &dataType, &pixelSize);
        ...
        switch (info.type) {
            case TextureType::TEX2D: {
                ...
                GL_CHECK(glTexSubImage2D(GL_TEXTURE_2D, 0, m_info.offset.x,
                                         m_info.offset.y, m_info.range.x,
                                         m_info.range.y, pixelFomrat, dataType,
                                         data));
                GL_CHECK(glBindTexture(GL_TEXTURE_2D, 0));
                break;
            }
            case TextureType::TEX2D_ARRAY: {
                CCASSERT(m_size == textureSize * m_info.range.z,
                         "update Texture size error");
                GL_CHECK(glBindTexture(GL_TEXTURE_2D_ARRAY, handle));
                GL_CHECK(glTexSubImage3D(
                    GL_TEXTURE_2D_ARRAY, 0, m_info.offset.x, m_info.offset.y,
            ...
            case TextureType::TEX3D: {
                ...
                GL_CHECK(glTexSubImage3D(
                    GL_TEXTURE_3D, 0, m_info.offset.x, m_info.offset.y,
                    m_info.offset.z, m_info.range.x, m_info.range.y,
                    m_info.range.z, pixelFomrat, dataType, data));
            ...
            case TextureType::CUBE: {
                ...
                for (int i = 0; i < 6; i++) {
                    ...
                    GL_CHECK(glTexSubImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i,
                                             0, 0, 0, m_info.range.x,
                                             m_info.range.y, pixelFomrat,
                                             dataType, pData));
                    offset += textureSize;
                }
